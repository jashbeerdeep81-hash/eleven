...
export const Route = createFileRoute("/sarees")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse a simple saree collection opened by JIYA voice commands." },
    ...
    <main className="min-h-dvh bg-background px-5 py-6 text-foreground sm:px-10">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-10 flex items-end justify-between gap-4">
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.16em] text-primary">JIYA style desk</p>
            <h1 className="mt-2 text-4xl font-bold tracking-tight">Saree collection</h1>
            <p className="mt-3 max-w-xl text-muted-foreground">A curated starting point for your next drape.</p>
          </div>
          <ShoppingBag className="hidden size-9 text-primary sm:block" />
        </div>
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {sarees.map((saree) => (
            <article key={saree.name} className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
              ...
              <div className="p-5">
                ...
                <Button className="mt-5 w-full" size="sm">Ask JIYA about this</Button>
              </div>
            </article>
          ))}
        </div>
